...
describe('ViewModeTabs', () => {
  ...
  it('renders both tab buttons', () => {
    render(<ViewModeTabs viewMode="table" onViewModeChange={mockOnViewModeChange} />);

    expect(screen.getByRole('button', { name: /switch to table view/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /switch to card view/i })).toBeInTheDocument();
  });

  it('has correct navigation role', () => {
    render(<ViewModeTabs viewMode="table" onViewModeChange={mockOnViewModeChange} />);

    const nav = screen.getByRole('navigation', { name: /view mode selection/i });
  ...
  it('applies active styles to table button when viewMode is table', () => {
    ...
    const tableButton = screen.getByRole('button', { name: /switch to table view/i });
    expect(tableButton).toHaveClass('bg-blue-100', 'text-blue-700', 'border-blue-200');
    expect(tableButton).toHaveAttribute('aria-pressed', 'true');

    const cardButton = screen.getByRole('button', { name: /switch to card view/i });
    expect(cardButton).toHaveClass('text-gray-500', 'hover:text-gray-700', 'hover:bg-gray-50');
    expect(cardButton).toHaveAttribute('aria-pressed', 'false');
  });

  it('applies active styles to card button when viewMode is cards', () => {
    render(<ViewModeTabs viewMode="cards" onViewModeChange={mockOnViewModeChange} />);

    const cardButton = screen.getByRole('button', { name: /switch to card view/i });
    expect(cardButton).toHaveClass('bg-blue-100', 'text-blue-700', 'border-blue-200');
    expect(cardButton).toHaveAttribute('aria-pressed', 'true');

    const tableButton = screen.getByRole('button', { name: /switch to table view/i });
    expect(tableButton).toHaveClass('text-gray-500', 'hover:text-gray-700', 'hover:bg-gray-50');
